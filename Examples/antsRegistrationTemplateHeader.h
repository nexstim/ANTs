...
DoRegistration(typename ParserType::Pointer & parser)
{
  ...
  if (initializeTransformsPerStageOption &&
      parser->Convert<bool>(initializeTransformsPerStageOption->GetFunction(0)->GetName()))
  {
    if (shouldCollapseBeDone)
    {
      ...
      {
        std::cerr
          << "ERROR: initialize-transforms-per-stage & collapse-output-transforms options are mutually exclusive."
          << std::endl;
      }
      return EXIT_FAILURE;
    }
  ...
  if (printSimilarityMeasureInterval && printSimilarityMeasureInterval->GetNumberOfFunctions())
  {
    unsigned int intervalLength =
      parser->Convert<unsigned int>(printSimilarityMeasureInterval->GetFunction(0)->GetName());
  ...
  if (initialMovingTransformOption && initialMovingTransformOption->GetNumberOfFunctions())
  {
    ...
    typename CompositeTransformType::Pointer compositeTransform =
      GetCompositeTransformFromParserOption<TComputeType, VImageDimension>(
  ...
  if (initialFixedTransformOption && initialFixedTransformOption->GetNumberOfFunctions())
  {
    ...
    typename CompositeTransformType::Pointer compositeTransform =
      GetCompositeTransformFromParserOption<TComputeType, VImageDimension>(
        parser, initialFixedTransformOption, isDerivedInitialFixedTransform);
    if (compositeTransform.IsNull())
    {
      return EXIT_FAILURE;
    }
  ...
  if (restoreStateOption && restoreStateOption->GetNumberOfFunctions())
  {
    ...
    typename CompositeTransformType::Pointer compositeTransform =
      GetCompositeTransformFromParserOption<TComputeType, VImageDimension>(
        parser, restoreStateOption, isDerivedInitialMovingTransform);
    if (verbose)
    {
      std::cout << "+" << std::endl;
    }
  ...
  for (int currentStage = numberOfTransforms - 1; currentStage >= 0; currentStage--)
  {
    ...
    if (convergenceOption.IsNotNull() && convergenceOption->GetNumberOfFunctions())
    {
      ...
      {
        iterations = parser->ConvertVector<unsigned int>(convergenceOption->GetFunction(currentStage)->GetParameter(0));
      }
      if (convergenceOption->GetFunction(currentStage)->GetNumberOfParameters() > 1)
      {
        convergenceThreshold = parser->Convert<RealType>(convergenceOption->GetFunction(currentStage)->GetParameter(1));
      }
      if (convergenceOption->GetFunction(currentStage)->GetNumberOfParameters() > 2)
      {
        convergenceWindowSize =
          parser->Convert<unsigned int>(convergenceOption->GetFunction(currentStage)->GetParameter(2));
        constexpr unsigned int minAllowedconvergenceWindowSize = 2; // The BSplineScatteredDataPoints requires at least
                                                                    // 2 points for interpolation.
        if (convergenceWindowSize < minAllowedconvergenceWindowSize)
        {
          if (verbose)
          {
            std::cerr << "Convergence Window Size must be greater than or equal to " << minAllowedconvergenceWindowSize
                      << std::endl;
          }
          return EXIT_FAILURE;
        }
    ...
    if (restrictDeformationOption.IsNotNull() &&
        restrictDeformationOption->GetNumberOfFunctions() > static_cast<unsigned int>(currentStage))
    {
    ...
    switch (xfrmMethod)
    {
      ...
      case RegistrationHelperType::BSplineDisplacementField:
      {
        ...
        {
          splineOrder = parser->Convert<unsigned int>(transformOption->GetFunction(currentStage)->GetParameter(3));
        }
        ...
        regHelper->AddBSplineDisplacementFieldTransform(
      ...
      case RegistrationHelperType::TimeVaryingVelocityField:
      {
        ...
        const float varianceForUpdateField =
          parser->Convert<float>(transformOption->GetFunction(currentStage)->GetParameter(2));
        const float varianceForUpdateFieldTime =
        ...
        const float varianceForTotalFieldTime =
          parser->Convert<float>(transformOption->GetFunction(currentStage)->GetParameter(5));
      ...
      case RegistrationHelperType::TimeVaryingBSplineVelocityField:
      {
        ...
        if (transformOption->GetFunction(currentStage)->GetNumberOfParameters() > 2)
        {
          numberOfTimePointSamples =
            parser->Convert<unsigned int>(transformOption->GetFunction(currentStage)->GetParameter(2));
        }
        unsigned int splineOrder = 3;
        if (transformOption->GetFunction(currentStage)->GetNumberOfParameters() > 3)
        {
        ...
        regHelper->AddTimeVaryingBSplineVelocityFieldTransform(
      ...
      case RegistrationHelperType::SyN:
      {
        ...
        {
          varianceForUpdateField = parser->Convert<float>(transformOption->GetFunction(currentStage)->GetParameter(1));
        }
      ...
      case RegistrationHelperType::BSplineSyN:
      {
        ...
        regHelper->AddBSplineSyNTransform(
          learningRate, meshSizeForTheUpdateField, meshSizeForTheTotalField, splineOrder);
      }
      break;
      case RegistrationHelperType::Exponential:
      {
        ...
        regHelper->AddExponentialTransform(
      ...
      case RegistrationHelperType::BSplineExponential:
      {
        ...
        {
          splineOrder = parser->Convert<unsigned int>(transformOption->GetFunction(currentStage)->GetParameter(4));
        }
        ...
        regHelper->AddBSplineExponentialTransform(
  ...
  for (int currentMetricNumber = numberOfMetrics - 1; currentMetricNumber >= 0; currentMetricNumber--)
  {
    ...
    if (!regHelper->IsPointSetMetric(currentMetric))
    {
      ...
      else if (currentMetric == RegistrationHelperType::Mattes || currentMetric == RegistrationHelperType::MI)
      {
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 3)
        {
          numberOfBins = parser->Convert<unsigned int>(metricOption->GetFunction(currentMetricNumber)->GetParameter(3));
        }
      }
    }
    else
    {
      if (whichMetric == "igdm")
      {
        ...
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 9)
        {
          gradientPointSetSigma =
            parser->Convert<RealType>(metricOption->GetFunction(currentMetricNumber)->GetParameter(9));
        }
        std::vector<unsigned int> neighborhoodRadius;
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 5)
        {
          neighborhoodRadius =
            parser->ConvertVector<unsigned int>(metricOption->GetFunction(currentMetricNumber)->GetParameter(5));
        }
        else
        {
        ...
        ReadImageIntensityPointSet<ImageType, MaskImageType, IntensityPointSetType>(fixedIntensityPointSet,
                                                                                    fixedFileName.c_str(),
                                                                                    fixedPointSetMaskFile.c_str(),
                                                                                    neighborhoodRadius,
                                                                                    gradientPointSetSigma);

        ReadImageIntensityPointSet<ImageType, MaskImageType, IntensityPointSetType>(movingIntensityPointSet,
                                                                                    movingFileName.c_str(),
                                                                                    movingPointSetMaskFile.c_str(),
                                                                                    neighborhoodRadius,
                                                                                    gradientPointSetSigma);
      ...
      else
      {
        ...
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 5)
        {
          pointSetSigma = parser->Convert<RealType>(metricOption->GetFunction(currentMetricNumber)->GetParameter(5));
        }
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 6)
        {
          evaluationKNeighborhood =
        ...
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 7)
        {
          alpha = parser->Convert<RealType>(metricOption->GetFunction(currentMetricNumber)->GetParameter(7));
        }
        if (metricOption->GetFunction(currentMetricNumber)->GetNumberOfParameters() > 8)
        {
        ...
        ReadLabeledPointSet<LabeledPointSetType>(
          fixedLabeledPointSet, fixedFileName.c_str(), useBoundaryPointsOnly, samplingPercentage);
        ReadLabeledPointSet<LabeledPointSetType>(
          movingLabeledPointSet, movingFileName.c_str(), useBoundaryPointsOnly, samplingPercentage);
...
#include "make_interpolator_snip.tmpl"
  ...
  if (saveStateOption && saveStateOption->GetNumberOfFunctions())
  {
          ...
          TransformType::TransformCategoryType::DisplacementField)
    {
      typename DisplacementFieldTransformType::Pointer oneToEndTransform =
        dynamic_cast<DisplacementFieldTransformType *>(
          savedStateTx->GetNthTransform(numStateComponents - 2).GetPointer());
      typename DisplacementFieldTransformType::Pointer endTransform = dynamic_cast<DisplacementFieldTransformType *>(
        savedStateTx->GetNthTransform(numStateComponents - 1).GetPointer());
      if (oneToEndTransform && oneToEndTransform->GetInverseDisplacementField() && endTransform &&
    ...
    //
    itk::ants::WriteTransform<TComputeType, VImageDimension>(savedStateCompositeTransform, saveStateFileName.c_str());
  }
  ...
  else // Write out each individual transform
  {
    ...
    for (unsigned int i = startIndex; i < numTransforms; ++i)
    {
      ...
      std::string transformTemplateName =
        RegTypeToFileName(currentTransformType, writeInverse, writeVelocityField, useMincFormat);
      ...
      if (writeVelocityField)
      {
        ...
        typedef
          typename RegistrationHelperType::TimeVaryingVelocityFieldTransformType TimeVaryingVelocityFieldTransformType;
